# Diagrama completo agrupado de la MT real

```mermaid
graph LR

  %% Nodos agrupados por subrutinas
  subgraph Inicio_e_Inicializacion[Inicio e Inicializacion]
    qStart["qStart"]
  ...
  subgraph Control_de_Bucle[Control de Bucle]
    qLoopSeekN["qLoopSeekN"]
    qGoBarForIter["qGoBarForIter"]
  end

  subgraph Limpieza_de_C_y_retorno[Limpieza de C y retorno]
    qClearC["qClearC"]
    qBackToBarFromHash["qBackToBarFromHash"]
  end

  subgraph Copia_de_A_hacia_C[Copia de A hacia C]
  ...
  subgraph Copia_de_B_hacia_C[Copia de B hacia C]
...
Este diagrama contiene todas las transiciones de la MT real, pero agrupadas por subrutinas para mejorar legibilidad.
Fue generado automáticamente desde `fib_tm_real.json`, sin eliminar ni modificar los diagramas anteriores.

## Qué hace cada agrupación

### 1) Inicio e Inicialización
- **Objetivo**: transformar la entrada unaria `1^n` en una estructura de trabajo para iterar Fibonacci.
- Marca la entrada como contador (`n`), crea separadores (`|`, `#`) y prepara los bloques base.
- Deja la cinta lista para entrar al ciclo principal.

### 2) Control de Bucle
- **Objetivo**: decidir si se debe ejecutar otra iteración de Fibonacci.
- Busca una marca `n` sin consumir; si la encuentra, la convierte a `x` (consumida) y continúa.
- Si ya no quedan `n`, salta a la fase de finalización.

### 3) Limpieza de C y retorno
- **Objetivo**: limpiar el bloque temporal `c` antes de calcular la nueva suma.
- Borra residuos de iteraciones anteriores y regresa al punto de arranque del cómputo.

### 4) Copia de A hacia C
- **Objetivo**: copiar el valor almacenado en `a` dentro del acumulador temporal `c`.
- Usa marcas auxiliares (`A`) para no perder posición durante el recorrido ida-vuelta.
- Al final restaura marcas y deja `a` consistente.

### 5) Copia de B hacia C
- **Objetivo**: completar `c = a + b` agregando también el contenido de `b`.
- Repite el patrón de copiado con marcas (`B`), escritura en `c`, y restauración.

### 6) Actualización de bloques
- **Objetivo**: avanzar la recurrencia Fibonacci una posición.
- Implementa lógicamente `a <- b` y `b <- c` mediante borrado/re-etiquetado de zonas.
- Termina dejando preparados los bloques para la siguiente vuelta.

### 7) Finalización
- **Objetivo**: convertir la cinta de trabajo en salida final limpia.
- Elimina símbolos auxiliares (`x`, `n`, separadores, marcas) y conserva solo el resultado en unario.
- Convierte el bloque final al formato de salida con `1` contiguos.
